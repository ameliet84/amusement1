...
if (n==0||n==1||base[i]=='+'||base[i]=='-'||(base[i]>=0 && base[i]<=32))
k=0;
...
if(k==1)
{
	while (i<n - 1 && k==1)
		...
		while (j<n && k==1)
			{
				if (base[j]=='+'||base[j]=='-'||base[i]==base[j]||(base[j]<=32 && base[j]>=0))
				{
				k=0;
				}
			++j;
...
{
int nbr=0, k=test_base(base), r, i=0, j=0, s=caracteres(str, 0), b=caracteres(base, 0);
